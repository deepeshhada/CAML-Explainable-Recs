...
class CFExperiment(Experiment):
    ...
    def _prepare_set_valid(self, data):
        ...
        for i in range(len(user)):
            ...
            if self.args.data_prepare == 1:
                if items[i] in self.ui_review_dict_valid[user[i]]:
                    user_reviews.append(self.ui_review_dict_valid[user[i]][items[i]])
                    user_concepts.append(self.ui_concept_dict_valid[user[i]][items[i]])
                    user_r_len.append(len(self.ui_review_dict_valid[user[i]][items[i]]))
                    user_c_len.append(len(self.ui_concept_dict_valid[user[i]][items[i]]))
                for x in self.ui_review_dict_valid[user[i]]:
                    if not x==items[i]:
                        user_reviews.append(self.ui_review_dict_valid[user[i]][x])
                        user_concepts.append(self.ui_concept_dict_valid[user[i]][x])
                        user_r_len.append(len(self.ui_review_dict_valid[user[i]][x]))
                        user_c_len.append(len(self.ui_concept_dict_valid[user[i]][x]))
                        if len(user_reviews) == self.args.dmax:
                            break
                if user[i] in self.iu_review_dict_valid[items[i]]:
                    item_reviews.append(self.iu_review_dict_valid[items[i]][user[i]])
                    item_concepts.append(self.iu_concept_dict_valid[items[i]][user[i]])
                    item_r_len.append(len(self.iu_review_dict_valid[items[i]][user[i]]))
                    item_c_len.append(len(self.iu_concept_dict_valid[items[i]][user[i]]))
                for x in self.iu_review_dict_valid[items[i]]:
                    if not x==user[i]:
                        item_reviews.append(self.iu_review_dict_valid[items[i]][x])
                        item_concepts.append(self.iu_concept_dict_valid[items[i]][x])
                        item_r_len.append(len(self.iu_review_dict_valid[items[i]][x]))
                        item_c_len.append(len(self.iu_concept_dict_valid[items[i]][x]))
    ...
    def load_review_data_valid (self, data_dir, data_type):
        lines_user_id = codecs.open('%s/valid_userid.txt' % data_dir, 'rb', 'utf-8').readlines()
        lines_item_id = codecs.open('%s/valid_itemid.txt' % data_dir, 'rb', 'utf-8').readlines()
        lines_review = codecs.open('%s/valid_%s.txt' % (data_dir, data_type), 'rb', 'utf-8').readlines()
    ...
    def _load_sets(self):
        ...
        if(self.no_text_mode==False):
            ...
            self.ui_review_dict_valid, self.iu_review_dict_valid = self.load_review_data_valid(data_link, "review")
            self.ui_concept_dict_valid, self.iu_concept_dict_valid = self.load_review_data_valid(data_link, "concepts")
            ...
            self.num_users_valid = max(self.ui_review_dict_valid.keys()) + 1
            self.num_items_valid = max(self.iu_review_dict_valid.keys()) + 1
        ...
        for epoch in range(1, self.args.epochs+1):
            ...
            if(epoch % self.args.eval==0):
                ...
                self._show_metrics(epoch, self.eval_dev,
                                    self.show_metrics,
                                        name='Dev')
                #print('See here metrics')
                #print(self._show_metrics.co_filename)
